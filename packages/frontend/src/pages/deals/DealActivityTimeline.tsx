...
import styles from './DealActivityTimeline.module.css';

type ActivityType = 'note' | 'call' | 'meeting' | 'message' | 'deal' | 'task';
...
const TYPE_CONFIG: Record<ActivityType, { icon: typeof MessageSquare; label: string; color: string }> = {
  ...
  deal: { icon: Calendar, label: 'Deal', color: 'var(--color-info-purple, #8b5cf6)' },
...
export function ActivityTimeline({ dealId }: DealActivityTimelineProps) {
  ...
  const fetchActivities = useCallback(async () => {
    ...
    setError('');
    try {
      const data = await api<{ entries: ActivityEntry[] }>(
        `/activity-logs?dealId=${dealId}`,
      );
      ...
      <div className={styles.container}>
        ...
        <div className={styles.emptyState}>
          ...
          <span className={styles.emptyDescription}>
            Activity from calls, meetings, and notes related to this deal will appear here.
          </span>
        </div>
      </div>
    );
